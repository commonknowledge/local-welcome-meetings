...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Calculate schedule
  const shiftPatterns =
    (await supabase.from<ShiftPattern>(`shiftpattern`).select('*')).data || []
  ...
  const calendar = ical({
    name: `${profile.firstName!}'s ADHD Together schedule`,
  })
  schedule.forEach((date) => {
    ...
    return calendar.createEvent({
      ...
      summary: `${
        date.isFillingIn ? '(Filling In)' : ''
      } ADHD Together session`,
  ...
  return calendar.serve(
    res,
    `adhdtogether-${profile.id}--${daysAhead}--${Date.now()}.ics`,
  )
}

...
export function ShiftSchedule() {
  ...
  const dates = useMemo(
    () => calculateSchedule(rota, maxDates, true),
    [rota, maxDates],
  )
...
export function SubscribeToCalendarDialog({
  open,
  onClose,
}: {
  open: boolean
      ...
      <div className='pointer-events-none fixed top-[10%] left-1/2 transform -translate-x-1/2 w-10/12 md:max-w-sm md:w-full z-50 inset-0 overflow-y-auto'>
        <div className='bg-white p-5 rounded-lg pointer-events-auto'>
          ...
          <Dialog.Description className='space-y-2 my-2'>
            <p>
              Copy this URL and subscribe to it in your calendar app, to keep
              track of the dates you are scheduled to attend.
            </p>
            <input
              type='email'
              disabled
              value={calendarURL}
              className='w-full text-xs border-none rounded-lg bg-gray-100'
            />
            <button className='button' onClick={() => copy(calendarURL)}>
              Copy URL{' '}
              <ClipboardCopyIcon className='w-4 h-4 text-inherit inline ml-[3px]' />
            </button>
          </Dialog.Description>
        </div>
      </div>
...
function DateManager({
  ...
  const fillInCount = shiftExceptions.filter(
    (se) => se.type === ShiftExceptionType.FillIn,
  ).length
  const fillInsNeeded = Math.max(0, dropOutCount - fillInCount)
  const allocationsRequired =
    shiftPattern.required_people - shiftAllocations.length
    ...
    <article className='flex flex-row w-full'>
      ...
      >
        <div className='text-xs uppercase'>
          {format(date, 'MMM', { timeZone: getTimezone() })}
        </div>
        <div className='text-2xl -mt-1'>
          {format(date, 'dd', { timeZone: getTimezone() })}
        </div>
        <div className='text-xs opacity-60 text-gray-500 font-semibold'>
          {format(date, 'h:mm a (zzz)', { timeZone: getTimezone() })}
        </div>
      </header>
      {/* Details */}
      <section className='text-left flex-grow col-span-4 w-full space-y-2'>
        {spStatus.notEnough && (
          <div
            className={`-mb-1 flex justify-between font-semibold uppercase w-full text-xs text-red-500`}
          >
            <span className='align-middle'>
              {n('more regulars', allocationsRequired, true)} needed -{' '}
              {shiftPattern.name} rota
            </span>
        ...
        {(shiftPattern.allowOneOffAllocations
          ...
          <div className='!-mb-1 text-red-500 text-xs font-semibold uppercase flex justify-between'>
            <span className='align-middle'>
              Temporary cover needed - this session
            </span>

...
export const CreateRoom: React.FunctionComponent<CreateRoomProps> = ({
  ...
  const suggestedSlug = useMemo(
    () => (name?.length ? slugify(name) : 'slug').trim().toLowerCase(),
    [name],
  )
        ...
        <div className='p-4 sm:p-5 bg-white'>
          ...
          <div className='grid grid-flow-row gap-2'>
            ...
            <div key='slug'>
              ...
              <div className='mt-1 flex rounded-md shadow-sm'>
                ...
                <input
                  ...
                  type='text'
                  {...register('slug', { pattern: /^[a-z](-?[a-z])*$/i })}
                  id='slug'
            ...
            <div key='slideshow'>
              ...
              <select
                {...register('slideshowName', { required: true })}
                id='slideshowName'
                className='mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md'
              >
            ...
            <div key='hubspot'>
              <label
                htmlFor='hubspotLeaderListId'
                className='block text-sm font-medium text-gray-700'
              >
                (Optional) Hubspot Leaders List ID
              </label>
              <input
                type='number'
                {...register('hubspotLeaderListId')}
                id='hubspotLeaderListId'
                className='mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md'
              />
              <p className='text-sm text-gray-500'>
                A Hubspot list for room-specific leaders. This is in addition to
                the global Hubspot leaders list. Numerical ID is found in the
                URL of the Hubspot list.
              </p>
            </div>
          </div>
        </div>

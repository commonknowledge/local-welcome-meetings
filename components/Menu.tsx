...
export function DropdownMenu({
  ...
  return (
    <Menu
      as='div'
      className='whitespace-nowrap relative inline-block text-left'
    >
      {({ open }) => (
        <Fragment>
          <div>
            {button || (
              <Menu.Button
                className={
                  'cursor-pointer opacity-80 bg-adhdDarkPurple rounded-lg p-2'
                }
              >
                <span>{title}</span>
                <ChevronDownIcon
    ...
    <Menu.Item>
      {({ active }) => (
        <span
          ...
          } group flex rounded-md cursor-pointer items-center w-full px-2 py-2 text-sm ${className}`}
          onClick={onClick}
        >
